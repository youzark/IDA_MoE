...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --lora-enable BOOL             Enable LoRA (default: $DEFAULT_LORA_ENABLE)"
            echo "  --lora-r NUM                   Set LoRA r (default: $DEFAULT_LORA_R)"
            echo "  --lora-alpha NUM               Set LoRA alpha (default: $DEFAULT_LORA_ALPHA)"
            echo "  --lora-dropout NUM             Set LoRA dropout (default: $DEFAULT_LORA_DROPOUT)"
            echo "  --lora-weight-path PATH        Set LoRA weight path (default: $DEFAULT_LORA_WEIGHT_PATH)"
            echo "  --lora-bias TYPE               Set LoRA bias type (default: $DEFAULT_LORA_BIAS)"
...
# Added LoRA defaults
lora_enable=${lora_enable:-$DEFAULT_LORA_ENABLE}
lora_r=${lora_r:-$DEFAULT_LORA_R}
lora_alpha=${lora_alpha:-$DEFAULT_LORA_ALPHA}
lora_dropout=${lora_dropout:-$DEFAULT_LORA_DROPOUT}
lora_weight_path=${lora_weight_path:-$DEFAULT_LORA_WEIGHT_PATH}
lora_bias=${lora_bias:-$DEFAULT_LORA_BIAS}
...
echo "LoRA Settings:"
echo "  Enable:           ${lora_enable}"
echo "  R:                ${lora_r}"
echo "  Alpha:            ${lora_alpha}"
echo "  Dropout:          ${lora_dropout}"
echo "  Weight Path:      ${lora_weight_path}"
echo "  Bias:             ${lora_bias}"
