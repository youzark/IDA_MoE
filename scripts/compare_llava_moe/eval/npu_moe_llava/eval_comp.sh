...
function run_from_config() {
    ...
    echo "Running evaluation from config file: $config_file in $mode mode"
}
...
function run_from_string() {
    ...
    # Read and store valid configurations from the string
    while IFS= read -r line; do  # Changed to handle lines properly
        [[ $line =~ ^#.*$ ]] && continue  # Skip commented lines
        [[ -z $line ]] && continue        # Skip empty lines
        temp_configs+=("$line")
    done < <(echo -e "$config_str")  # Use process substitution with echo -e
...
function print_usage() {
    echo "Usage:" >&2
    echo "  1. $0 --config <config_file> [--mode <mode>]" >&2
    echo "  2. $0 --config-str <config_string> [--mode <mode>]" >&2
    echo "  3. $0 --model_paths <path1> [path2 ...] --datasets <dataset1> [dataset2 ...] [--mode <mode>]" >&2
    echo >&2
    echo "Options:" >&2
...
if [ -n "$config_file" ]; then
    ...
    # Generate config string and run
