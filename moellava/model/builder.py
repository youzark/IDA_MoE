...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto",
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and 'moe' not in model_name.lower() and model_base is not None:
            ...
            if 'qwen' in model_base.lower() and '.5' not in model_base.lower():
            ...
            elif 'qwen' in model_base.lower() and '.5' in model_base.lower():
            ...
            if 'qwen' in model_name.lower() and '.5' not in model_name.lower():
            ...
            elif 'qwen' in model_name.lower() and '.5' in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, padding_side=padding_side)
            ...
            if not merge:
                import deepspeed
                deepspeed.init_distributed(dist_backend=DISTBACKEND)
            ...
            """
            LLM is specified seperately with model_base, which means the model_path specify the mm_projector path
            """
            ...
            # =============================================================================================
                ...
                # model.config.eos_token_id = tokenizer.eos_token_id
            elif 'qwen' in model_name.lower() and '.5' not in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, padding_side=padding_side)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
                if getattr(cfg_pretrained, 'moe', {}).get('moe_enable', False):
            ...
            """
            no PEFT, just normal model after finetuning
            """
            if 'mpt' in model_name.lower():
                ...
                from moellava.model.language_model.qwen2.tokenization_qwen2 import Qwen2Tokenizer
                tokenizer = Qwen2Tokenizer.from_pretrained(model_path, use_fast=False, padding_side=padding_side)
                if 'moe' in model_name.lower():
                    ...
                    model = MoELLaVAQWen2ForCausalLM.from_pretrained_unified(pretrained_model_name_or_path=model_path, low_cpu_mem_usage=True, **kwargs)
                    import deepspeed
                    deepspeed.init_distributed(dist_backend=DISTBACKEND)
                    # Initialize the DeepSpeed-Inference engine
                    ds_engine = deepspeed.init_inference(model,
                ...
                if 'moe' in model_name.lower():
                    ...
                    # model = EvalMoELLaVAQWenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
                    from moellava.model.language_model.modeling_qwen_moe import MoELLaVAQWenForCausalLM
                    model = MoELLaVAQWenForCausalLM.from_pretrained_unified(pretrained_model_name_or_path=model_path, low_cpu_mem_usage=True, **kwargs)
                    import deepspeed
                    deepspeed.init_distributed(dist_backend=DISTBACKEND)
                    # Initialize the DeepSpeed-Inference engine
                    ds_engine = deepspeed.init_inference(model,
                ...
                if 'moe' in model_name.lower():
                    ...
                    model = MoELLaVAPhiForCausalLM.from_pretrained_unified(pretrained_model_name_or_path=model_path, low_cpu_mem_usage=True, **kwargs)
                    import deepspeed
                    deepspeed.init_distributed(dist_backend=DISTBACKEND)
                    # Initialize the DeepSpeed-Inference engine
                    ds_engine = deepspeed.init_inference(model,
            ...
            #     model.config.eos_token_id = tokenizer.eos_token_id
                ...
                if 'moe' in model_name.lower():
                    ...
                    model = MoELLaVAStablelmForCausalLM.from_pretrained_unified(pretrained_model_name_or_path=model_path, low_cpu_mem_usage=True, **kwargs)
                    import deepspeed
                    deepspeed.init_distributed(dist_backend=DISTBACKEND)
                    # Initialize the DeepSpeed-Inference engine
                    ds_engine = deepspeed.init_inference(model,
    ...
    if 'llava' in model_name.lower():
        ...
        if model.config.mm_image_tower is not None:
            ...
            image_tower = image_tower.to(dtype=torch.float16).npu()
        ...
        if model.config.mm_video_tower is not None:
            ...
            video_tower = video_tower.to(dtype=torch.float16).npu()
