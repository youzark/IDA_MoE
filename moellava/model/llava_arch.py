...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        # print("/Users/yannhua/moe/moellava/model/llava_arch.py\n",f"LlavaMetaModel Config: {config}")
        if getattr(config, "mm_image_tower", None) is not None:
            self.image_tower = build_image_tower(config, delay_load=False)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        # contain_nan("image feature after mlp", image_features)
...
def init(self, image_tower, args, delay_load=False, cache_dir='./cache_dir'):
super().init()
self.is_loaded = False
    ...
    if not delay_load:
        ...
        # self.cfg_only = CLIPVisionConfig.from_pretrained(self.cache_dir)

def load_model(self):
    # self.image_processor = CLIPImageProcessor.from_pretrained(self.cache_dir)
    # self.image_tower = CLIPVisionModel.from_pretrained(self.cache_dir)
