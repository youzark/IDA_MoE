...
from transformers.models.gemma2.modular_gemma2 import Gemma2Config, Gemma2DecoderLayer,  Gemma2Model,  Gemma2ForCausalLM
...
class MoELLaVAGemma2Config(Gemma2Config):
    ...
    **kwargs
        ...
        super(MoELLaVAGemma2Config, self).__init__(**kwargs)
...
class MoEGemma2DecoderLayer(Gemma2DecoderLayer):
    def __init__(
            self, 
            config: MoELLaVAGemma2Config, 
            layer_idx:int,
            *args, 
            **kwargs
        ):
...
class MoELLAVAGemma2Model(LlavaMetaModel, Gemma2Model):
    config_class = MoELLaVAGemma2Config
    def __init__(
            self, 
            config:  MoELLaVAGemma2Config,
        ):
        super(MoELLAVAGemma2Model, self).__init__(config)
        self.is_moe_initialized = False
        for i in range(config.num_hidden_layers):
            self.layers[i] = MoEGemma2DecoderLayer(config = config, layer_idx= i)
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_moe_metrics, all_moe_loss] if v is not None)
...
class MoELLaVAPhiForCausalLM(Gemma2ForCausalLM, LlavaMetaForCausalLM):
    config_class = MoELLaVAGemma2Config

    def __init__(self, config):
        super(Gemma2ForCausalLM, self).__init__(config)
        self.model = MoELLAVAGemma2Model(config)
    ...
    def from_pretrained_unified(
        ...
        **kwargs,
        ...
        if config is not None:
            assert isinstance(config, MoELLaVAGemma2Config), f"Config Provided to MoELLaVAPhiForCausalLM.from_pretrained_unified should be MoELLaVAPhiConfig be get {type(config)}"
...
AutoConfig.register("moe_llava_phi", MoELLaVAGemma2Config)
AutoModelForCausalLM.register(MoELLaVAGemma2Config, MoELLaVAPhiForCausalLM)
