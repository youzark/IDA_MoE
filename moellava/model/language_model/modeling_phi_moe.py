...
from .phi.modeling_phi import PhiModel, PhiForCausalLM, PhiMLP, PhiDecoderLayer
...
class MoEPhiDecoderLayer(PhiDecoderLayer):
    def __init__(
        self, 
        config:MoELLaVAPhiConfig, 
        layer_idx:int,
        *args, 
        **kwargs
    ):
...
class MoELLaVAPhiModel(LlavaMetaModel, PhiModel):
    ...
    def __init__(self, config: PhiConfig):
        ...
        for i in range(config.num_hidden_layers):
            self.layers[i] = MoEPhiDecoderLayer(config = config, layer_idx= i)
    ...
    def from_pretrained_unified(
        ...
        **kwargs,
        ...
        if config is not None:
            assert isinstance(config, MoELLaVAPhiConfig), f"Config Provided to MoELLaVAPhiForCausalLM.from_pretrained_unified should be MoELLaVAPhiConfig be get {type(config)}"
