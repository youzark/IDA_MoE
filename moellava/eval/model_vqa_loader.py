...
import math
import torch_npu
from moellava.eval.moe_generate_util import generate_with_moe_metrics 
...
def eval_model(args):
    ...
    answers_file_base = os.path.basename(answers_file)
    dir_name = os.path.dirname(answers_file)
    if 'chunk' in answers_file_base and '.jsonl' in answers_file_base:
        idx = answers_file_base.replace('chunk', '').split('.')[0]  # Extract the index
        cv_answers_file = os.path.join(dir_name, f"chunk_cv_{idx}.json")
    else:
        # Fallback if the naming pattern is different
        cv_answers_file = answers_file.replace('.jsonl', '_cv.json')

    cv_ans_file = open(cv_answers_file, "w")
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        image_tensor = image_tensor.to(dtype=torch.float16).npu()
        ...
        with torch.inference_mode():
            ...
            # output = generate_with_moe_metrics(
            #     model,
            #     input_ids,
            #     images=image_tensor,
            #     do_sample=True if args.temperature > 0 else False,
            ...
            # Now you can access both the generated sequences and MoE metrics
            # output_ids = output['sequences']
            # moe_metrics = output['moe_metrics_list']
            # print(moe_metrics)
            with torch.no_grad():
                forward_output = model(output_ids, images=image_tensor)
                cvs = [ metrics["cv"] for metrics in forward_output["moe_metrics_list"] if metrics and "cv" in metrics]
                # cvs = [metric["cv"] for metric in forward_output["moe_metrics_list"]]
                cv_list += cvs
                print(cvs)
        ...
        ans_file.write(json.dumps({"question_id": idx,
                                   ...
                                   "model_id": model_name,
                                   "metadata": {}}) + "\n")
    cv_ans_file.write(json.dumps(cv_list))
    cv_ans_file.close()
        # ans_file.flush()
    ans_file.close()
    print(f"CV Mean {sum(cv_list)/ float(len(cv_list))}")
