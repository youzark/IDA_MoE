...
from moellava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    ...
    MAX_VIDEO_LENGTH, IMAGE_EMBEDING_TOKEN_SIZE
...
from moellava.model.language_model.llava_qwen2 import LlavaQWen2Config, LlavaQWen2ForCausalLM
...
def print_memory():
    rank0_print(f"Memory allocated :{torch.npu.memory_allocated()/1e9:.2f}GB")
    rank0_print(f"Memory reserved :{torch.npu.memory_reserved()/1e9:.2f}GB")
...
class ModelArguments:
    ...
    components_per_expert: int = 1,
    routing_dim: int = 32,
    router_aux_loss_coef: float = 0.01
    group_reactivation: bool = True,

    # LoRA MoE(Use LoRA as Expert rather than Normal LoRA on regular parameter)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    routing_file_path: str = "none"
    if_training_for_routing_analysis: bool = True
...
def safe_save_config(config, output_dir):
    ...
    if os.path.exists(config_path):
        try:
            ...
            # If existing config is valid, no need to save again
            return
        except json.JSONDecodeError:
            # If existing config is invalid, we'll overwrite it
            pass
...
def preprocess_multimodal(
    ...
    """
    Truncate extra images to images limits.
    add im_start(vid_start) and im_end(vid_end)
    """
    is_multimodal = data_args.is_multimodal
    if not is_multimodal:
        return sources

    for source in sources: ## sources: list of conversations
        for sentence in source: ## source: conversation, list of sentences : {"from":, "value":}
...
def preprocess_qwen_v1(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        if cur_len < tokenizer.model_max_length:
            if cur_len != total_len:
                print("Conversations with Problem")
                print(conversations)
                print("Input Ids")
                print(input_ids)
                print("Targets")
...
def preprocess_stablelm(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
            # target[cur_len : cur_len + instruction_len] = tokenizer.pad_token_id

            cur_len += round_len
        target[cur_len:] = IGNORE_INDEX
        # target[cur_len:] = tokenizer.pad_token_id
...
def preprocess_phi(
    ...
    for conversation, target in zip(conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
        total_len += conversation.count(tokenizer.pad_token) ## because <|endoftext|> both severs as "padding" and "round seperator", when it servers as "round seperator" it should be take in to account of total len
...
def preprocess_qwen(
    ...
    for i, source in enumerate(sources):
        ...
        # Convert to tensor and append to lists
        input_ids_list.append(torch.tensor(input_id, dtype=torch.long))
...
def preprocess_plain(
    ...
    """
    prerequisite:
        1. only two sentence: one human , one agent
        2. the first token of human ask is image_token
    return:
        truncate human question, only return "image_token {bot_reply}"
    """
    # add end signal and concatenate together
    conversations = []
    for source in sources: ## source : a conversation , a list of dict {"from":, "value":}
        assert len(source) == 2 ## one question , one answer
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("stablelm"):  
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        for data in data_path:
            dataset_name = data.split('/')[-1].split('.')[0]
            data = json.load(open(data, "r"))
            for i in data:
                i["specifier"] = f"{dataset_name}--{i['id']}"
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # input_ids = input_ids[:, :self.tokenizer.model_max_length - IMAGE_EMBEDING_TOKEN_SIZE]
        # labels = labels[:, :self.tokenizer.model_max_length - IMAGE_EMBEDING_TOKEN_SIZE]
        input_ids = input_ids[:, :self.tokenizer.model_max_length]
        labels = labels[:, :self.tokenizer.model_max_length]
        ids = [ instance["specifier"] for instance in instances]
        batch = dict(
            input_ids=input_ids,
            labels=labels,
...
def train():
    ...
    bnb_model_from_pretrained_args = {} ## bit and byte
    ...
    rank0_print("Before Loading Memory")
    print_memory()
    ...
    if model_args.image_tower is not None or model_args.video_tower is not None:
        if not model_args.moe_enable:
            ...
            # elif 'qwen' in model_args.model_name_or_path.lower() and '.5' in model_args.model_name_or_path.lower():
            ...
            if 'qwen2' in model_args.model_name_or_path.lower():
                from moellava.model.language_model.modeling_qwen2_moe import MoELLaVAQWen2ForCausalLM
                model = MoELLaVAQWen2ForCausalLM.from_pretrained_unified(
                    pretrained_model_name_or_path=model_args.model_name_or_path,
                    model_args=model_args,
                    **bnb_model_from_pretrained_args
                )
            elif 'qwen' in model_args.model_name_or_path.lower():
                from moellava.model.language_model.modeling_qwen_moe import MoELLaVAQWenForCausalLM
                model = MoELLaVAQWenForCausalLM.from_pretrained_unified(
                    pretrained_model_name_or_path=model_args.model_name_or_path,
                    model_args=model_args,
                    **bnb_model_from_pretrained_args
                )
            # elif 'gemma' in model_args.model_name_or_path.lower():
            #     from moellava.model.language_model.modeling_gemma2_moe import MoELLaVAGemma2ForCausalLM
            #     model = MoELLaVAGemma2ForCausalLM.from_pretrained_unified(
            #         pretrained_model_name_or_path=model_args.model_name_or_path,
            #         model_args=model_args,
            #         **bnb_model_from_pretrained_args
            #     )
            elif 'phi' in model_args.model_name_or_path.lower():
                from moellava.model.language_model.modeling_phi_moe import MoELLaVAPhiForCausalLM
                model = MoELLaVAPhiForCausalLM.from_pretrained_unified(
                    pretrained_model_name_or_path=model_args.model_name_or_path,
                    model_args=model_args,
                    **bnb_model_from_pretrained_args
                )
                ...
                from moellava.model.language_model.modeling_stablelm_moe import MoELLaVAStablelmForCausalLM
                model = MoELLaVAStablelmForCausalLM.from_pretrained_unified(
                    pretrained_model_name_or_path=model_args.model_name_or_path,
                    model_args=model_args,
                    **bnb_model_from_pretrained_args
                )
    ...
    if training_args.gradient_checkpointing:
        """
        memory / compute time trade-off for large model training on less sufficent GPU memories.
        """
    ...
    rank0_print("After Loading Memory")
    print_memory()

    """
    Since Changing to New moe impl, this init thing is not needed, and peft still not considered feasible with moe at current stage, so this legacy code abandoned
    """
    # if model_args.moe_enable:
    #     if training_args.lora_enable:
    #         from peft import LoraConfig, get_peft_model
    #         if 'qwen' in model_args.model_name_or_path.lower() and '.5' not in model_args.model_name_or_path.lower():
    #             target_modules = [
    ...
    if 'mpt' in model_args.model_name_or_path:
        ...
        if 'qwen2' in model_args.model_name_or_path.lower():
            ...
            tokenizer.pad_token = '<|reg0|>'
            tokenizer.pad_token_id = tokenizer.special_tokens["<|reg0|>"]
        # elif 'gemma' in model_args.model_name_or_path.lower():
        #     tokenizer = transformers.AutoTokenizer.from_pretrained(
        #         model_args.model_name_or_path,
        #         model_max_length=training_args.model_max_length,
    ...
    trainer = LLaVATrainer(
        ...
        tokenizer=tokenizer,
        args=training_args,
        routing_file_path = training_args.routing_file_path,
        training = training_args.if_training_for_routing_analysis,
        **data_module
        )
    ...
    if model_args.moe_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            safe_save_config(model.config, training_args.output_dir)
